...
import { Plus, X, Save, Pencil, Trash2, Users, RefreshCw } from "lucide-react";
...
const roleStyle = (r: string) =>
  r === "مدير النظام" ? { backgroundColor: "#fce7f3", color: "#be185d" } :
  r === "مهندس" ? { backgroundColor: "#dbeafe", color: "#1565C0" } :
  r === "فني" ? { backgroundColor: "#fef3c7", color: "#d97706" } :
...
export default function UsersPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setEditing(null);
      load();
    } catch { setMsg("خطأ في الحفظ. تأكد من عدم تكرار البريد الإلكتروني أو الرقم الوظيفي."); }
  ...
  return (
    <div className="p-6 lg:p-8" dir="rtl">
      ...
      {/* Search */}
      <div className="mb-4">
        <input className={inp} value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالاسم أو الرقم الوظيفي أو البريد..." />
      </div>
      ...
      {editing && (
        <div className="bg-white rounded-3xl p-6 mb-6 shadow-lg border border-blue-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">الاسم الكامل <span className="text-red-500">*</span></label>
              <input className={inp} value={editing.name} onChange={e => set("name", e.target.value)} placeholder="محمد أحمد" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">الرقم الوظيفي <span className="text-red-500">*</span></label>
              <input className={inp} value={editing.employeeId} onChange={e => set("employeeId", e.target.value)} placeholder="EMP-001" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">البريد الإلكتروني <span className="text-red-500">*</span></label>
            ...
            <div>
              ...
              <input className={inp} value={editing.phone || ""} onChange={e => set("phone", e.target.value)} placeholder="+218 91 000 0000" />
            </div>
            <div>
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={save} disabled={saving || !editing.name || !editing.employeeId || !editing.email || (!editing.id && !editing.password)}
      ...
      {/* Table */}
      {loading ? (
        <div className="space-y-2">{Array(3).fill(0).map((_, i) => <div key={i} className="animate-pulse bg-gray-200 rounded-2xl h-16" />)}</div>
      ) : filtered.length === 0 ? (
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((u: any) => (
                <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3"><span className="text-xs px-2 py-1 rounded-full font-medium" style={roleStyle(u.role)}>{u.role}</span></td>
                  <td className="px-4 py-3">
                    <span className="text-xs px-2 py-1 rounded-full font-medium"
                      style={{ backgroundColor: u.status === "نشط" ? "#dcfce7" : "#fee2e2", color: u.status === "نشط" ? "#16a34a" : "#dc2626" }}>
                      {u.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
